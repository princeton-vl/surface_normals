...
local function angle_diff(input, target)
    -- to test
    local cos = torch.sum(torch.cmul(input, target),1)      -- dot product of normals , seems quite expensive move
...
function evaluate( data_loader, model, criterion, max_n_sample ) 
    ...
    for iter = 1, n_normal_iters do
        ...
        -- get the loss 
        total_normal_validation_loss = total_normal_validation_loss + batch_loss * 320*240        -- 

        -- update the number of point pair
        n_total_normal_point = n_total_normal_point + 320*240
