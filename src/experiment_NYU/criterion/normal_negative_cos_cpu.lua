...
local normal_negative_cos, parent = torch.class('nn.normal_negative_cos_cpu', 'nn.Criterion')
...
function normal_negative_cos:updateOutput(input, target)
    ...
    local cpu_input = input:double()        -- is this necessary?  can it be gpu data??    to check

    for batch_idx = 1 , cpu_input:size(1) do

        n_point_total = n_point_total + target[batch_idx].n_point

        local x_arr = target[batch_idx].x:long()            -- to check: the length of x vary with each sample!!!!! 
        local y_arr = target[batch_idx].y:long()        
...
function normal_negative_cos:updateGradInput(input, target)    
    ...
    for batch_idx = 1 , cpu_input:size(1) do
        ...
        local unsqueeze = nn.Unsqueeze(2):forward( target[batch_idx].normal )

        local p2 = torch.Tensor(3, cpu_input:size()[3], target[batch_idx].n_point):zero()
        local p1 = torch.Tensor(batch_input:size(1), batch_input:size(2), batch_input:size(3)):zero()
