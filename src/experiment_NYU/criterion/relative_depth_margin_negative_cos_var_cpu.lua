...
-- sub criterions
require './relative_depth_margin_cpu'
require './normal_negative_cos_cpu'

local relative_depth_margin_negative_cos_var_cpu, parent = torch.class('nn.relative_depth_margin_negative_cos_var_cpu', 'nn.Criterion')

function relative_depth_margin_negative_cos_var_cpu:__init(w_normal, margin, d_var_thresh)
    print("\n>>>>>>>>>>>>>>>>>>>>>>Criterion: relative_depth_margin_negative_cos_var_cpu()")

    parent.__init(self)
    self.depth_crit = nn.relative_depth_crit_cpu(margin)
...
function relative_depth_margin_negative_cos_var_cpu:updateOutput(input, target)
...
function relative_depth_margin_negative_cos_var_cpu:updateGradInput(input, target)
    -- the input is tensor taht represents the depth map
    -- the target is a table, where the first component is a table that contains relative depth info, and the second component is a table that contains normal info.
