...
function relative_depth_crit:_loss_func_arr(z_A, z_B, ground_truth)
    ...
    local val1 = torch.cmax(torch.ones(#z_A_z_B):fill(0), torch.cmul(z_A_z_B, z_A_z_B))
    local val2 = torch.log(torch.ones(#z_A_z_B) + torch.exp(torch.cmul(z_A_z_B, ground_truth) * -1.0))
    return torch.add(torch.cmul(val2, mask), torch.cmul(val1, torch.add(torch.ones(#mask), -mask)))
end
