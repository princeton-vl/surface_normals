...
function relative_depth_crit:__init(margin)
    print(string.format("\t\tCriterion: relative_depth_margin, margin = %f", margin))
    parent.__init(self)
...
function relative_depth_crit:_loss_func_arr(z_A, z_B, ground_truth)
    ...
    -- local ones_cuda_tensor = torch.ones(#log_z_A_div_z_B):cuda()
    -- local val1 = torch.cmax(ones_cuda_tensor, torch.cmul(log_z_A_div_z_B, log_z_A_div_z_B))
    -- log_z_A_div_z_B:cmul(ground_truth)  -- account for the difference between z_A/z_B and z_B/z_A
    -- log_z_A_div_z_B:cmin(ones_cuda_tensor, log_z_A_div_z_B)
    -- local val2 = torch.log(ones_cuda_tensor + torch.exp(log_z_A_div_z_B * -1.0))
    -- return torch.add(torch.cmul(val2, mask), torch.cmul(val1, torch.add(ones_cuda_tensor, -mask)))


    local mask = torch.abs(ground_truth)
    local z_A_z_B = torch.add(z_A, -z_B)
    local ones_cuda_tensor = torch.ones(#z_A_z_B):cuda()
    local margin_cuda_tensor = torch.Tensor(#z_A_z_B):fill(self.margin):cuda()
    local margin2_cuda_tensor = torch.Tensor(#z_A_z_B):fill(self.margin*self.margin):cuda()
    local val1 = torch.cmax(margin2_cuda_tensor, torch.cmul(z_A_z_B, z_A_z_B))
    z_A_z_B:cmul(ground_truth)
    z_A_z_B:cmin(margin_cuda_tensor, z_A_z_B)
    local val2 = torch.log(ones_cuda_tensor + torch.exp(z_A_z_B * -1.0))
    return torch.add(torch.cmul(val2, mask), torch.cmul(val1, torch.add(ones_cuda_tensor, -mask)))
end
