...
function DataLoader:parse_depth_and_normal(relative_depth_filename, normal_filename)
    ...
    if normal_filename ~= nil then
        ...
        local normal_hdf5_filename = string.gsub(normal_filename, ".csv", ".h5");
        self.normal_handle.hdf5_handle = hdf5.open(normal_hdf5_filename,'r')
    else
        self.normal_handle = {}
    end
...
function DataLoader:load_indices( depth_indices, normal_indices, b_load_gtz )
    ...
    for i = n_depth + 1 , batch_size do
        ...
        -- 1. rescale the image 
        -- 2. rescale the keypoint coordinate
        local orig_img = image.load(img_name)
        local orig_img_height = orig_img:size(2)
        local orig_img_width = orig_img:size(3)
        local rescaled_x = math.ceil(self.normal_handle[idx].x * g_input_width / orig_img_width)
        local rescaled_y = math.ceil(self.normal_handle[idx].y * g_input_height / orig_img_height)
        -- print(string.format("orig (x,y)=%d,%d, rescaled (x,y)=%d,%d, orig(width, height)=%d,%d", self.normal_handle[idx].x, self.normal_handle[idx].y, rescaled_x, rescaled_y, orig_img_width, orig_img_height))
        local img = image.scale(orig_img, g_input_width, g_input_height)        --image.scale(src, width, height, [mode])       
        ...
        local _this_normal_vector  = self.normal_handle.hdf5_handle:read('/normal'):partial({1, 3}, {idx, idx})
        _this_normal_vector = _this_normal_vector:cuda()
        _this_normal_vector[{{1,2}}]:mul(-1)
        -- print(torch.sum(torch.pow(_this_normal_vector,2)))



        -- pay atteniton to the order!
        _batch_target_normal_gpu[i - n_depth].x:resize(n_point):fill( rescaled_x  )      -- n_point = 1, use fill() 
        _batch_target_normal_gpu[i - n_depth].y:resize(n_point):fill( rescaled_y  )
        _batch_target_normal_gpu[i - n_depth].normal = _this_normal_vector:clone()                      
