...
local relative_depth_negative_cos_focal, parent = torch.class('nn.relative_depth_negative_cos_focal_cpu', 'nn.Criterion')
...
function relative_depth_negative_cos_focal:updateOutput(input, target)
    ...
    self.output = 0
    if n_depth > 0 then
        self.output = self.output + self.depth_crit:forward(input[1]:sub(1, n_depth), target[1])
    end
    if n_normal > 0 then
