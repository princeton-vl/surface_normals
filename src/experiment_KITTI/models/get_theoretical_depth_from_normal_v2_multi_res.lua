...
function ABC_sum:__init(mode, scale)
	parent.__init(self)      
   ...
   self.constant_X_Y_1 = torch.Tensor(3, height, width)
   ...
   elseif mode == 'down' then
	...
	for y = 1 , height do
	   for x = 1 , width do
	      self.constant_X_Y_1[{1,y,x}] = (x + _offset_x - _cx_rgb) / _fx_rgb
	      self.constant_X_Y_1[{2,y,x}] = (y + _offset_y - _cy_rgb) / _fy_rgb
	   end
	end
...
function get_theoretical_depth_multi_res( scale )
   ...
   local ABC_right = nn.ABC_sum('right', scale)(normal_input):annotate{name = 'ABC_right'}
   local ABC_left = nn.ABC_sum('left', scale)(normal_input):annotate{name = 'ABC_left'}
   local ABC_down = nn.ABC_sum('down', scale)(normal_input):annotate{name = 'ABC_down'} 
   local ABC_up = nn.ABC_sum('up', scale)(normal_input):annotate{name = 'ABC_up'}
