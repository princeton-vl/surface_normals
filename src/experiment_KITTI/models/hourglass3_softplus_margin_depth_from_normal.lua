...
function get_model()
    ...
    --Enforce the output depth to be positive or 0
    model:add(nn.SoftPlus(true))
    model:add(nn.AddConstant(1.0199491977692,false))

    return model 
end


require('../criterion/relative_depth_margin_normal_depth')
function get_criterion()
    return nn.relative_depth_margin_normal_depth(g_args.w_n, g_args.margin)
end
