...
for scale_idx = 1, g_args.n_scale do
    _batch_target_normal_gpu[scale_idx] = {}
    for i = 1 , g_args.bs  do         
        _batch_target_normal_gpu[scale_idx][i] = {}
        _batch_target_normal_gpu[scale_idx][i].x = torch.CudaTensor()
        _batch_target_normal_gpu[scale_idx][i].y = torch.CudaTensor()
        _batch_target_normal_gpu[scale_idx][i].normal = torch.CudaTensor()
    end
end
...
function DataLoader:load_indices( depth_indices, normal_indices, b_load_gtz )
    ...
    if n_depth > 0 then
        _batch_target_relative_depth_gpu.gt_depth:resize(batch_size, 480, 640)
    end
    if n_normal > 0 then
        _batch_target_normal_gpu.gt_depth:resize(batch_size, 480, 640)
    end
    ...
    for i = 1, n_depth do    
        ...
        if b_load_gtz then
            ...
            if path.exists(gt_z_filename) then                
                ...
                -- print(gt_z_filename)
                _batch_target_relative_depth_gpu.gt_depth[{i,{}}]:copy(gtz_h5_handle:read('/gt_depth'):all())
                gtz_h5_handle:close()
            else
                print("File not found:", gt_z_filename)
                _batch_target_relative_depth_gpu.gt_depth[{i,{}}]:copy(torch.rand(480,640))
            end
    ...
    for i = n_depth + 1 , batch_size do
        ...
        if b_load_gtz then
            ...
            if path.exists(gt_z_filename) then
                local gtz_h5_handle = hdf5.open(gt_z_filename, 'r')
                -- print(gt_z_filename)
                _batch_target_normal_gpu.gt_depth[{i - n_depth,{}}]:copy(gtz_h5_handle:read('/gt_depth'):all())
                gtz_h5_handle:close()
            else
                print("File not found:", gt_z_filename)
        ...
        local n_point_at_scales = {math.ceil(n_point / 81 * 64), math.ceil(n_point / 81 * 16)}
        n_point_at_scales[3] = math.max(n_point - n_point_at_scales[1] - n_point_at_scales[2], 1)

        for scale_idx = 1, g_args.n_scale do
            local normal_name
            if scale_idx == 1 then
                normal_name = string.gsub(img_name, ".png", "_normal.bin")
            else
                normal_name = string.gsub(img_name, ".png", string.format("_normal_scale%d.bin", g_scales[scale_idx]))
            end
             
            -- local this_n_point = (320 / g_scales[scale_idx] - 2) * (240  / g_scales[scale_idx] - 2)
            local this_n_point = math.min(n_point_at_scales[scale_idx], (320 / g_scales[scale_idx] - 2) * (240  / g_scales[scale_idx] - 2))

            local file = torch.DiskFile(normal_name, 'r'):binary();    
            local normal = torch.DoubleTensor(file:readDouble(5 * this_n_point))
            file:close();                
            normal = torch.view(normal, this_n_point, 5)                 -- tested
            normal = normal:t()
            normal = normal:cuda()

            -- pay atteniton to the order!
            _batch_target_normal_gpu[scale_idx][i - n_depth].x:resize(this_n_point):copy(normal[{1,{}}]:int())
            _batch_target_normal_gpu[scale_idx][i - n_depth].y:resize(this_n_point):copy(normal[{2,{}}]:int())
            _batch_target_normal_gpu[scale_idx][i - n_depth].normal = normal[{{3,5},{}}]:clone()       -- To check.  Do we need to clone()? 
            _batch_target_normal_gpu[scale_idx][i - n_depth].n_point = this_n_point
        end
